...
3. It reads a Cezar-owned grant store, not a manifest-owned approval field. A package requesting no permissions receives the empty grant automatically. A package requesting permissions remains inactive until the user approves the exact requested set in the UI.
4. The backend exposes validated inventory and approved frontend assets through versioned, same-origin routes. In hosted mode it exposes no local package metadata or code and returns a non-error unavailable result for discovery.
...
|---|---|---|---|
| Q1 | Should the backend execute discovered extension code, or should it only discover and validate while the browser host performs runtime registration? | **Backend discovery/validation; browser runtime registration.** The backend never imports extension code. The browser imports only an admitted, explicitly approved frontend entrypoint and registers it through `packages/web/src/extensions/registry.ts`. **⚠ NEEDS HUMAN CONFIRMATION** | This is the smallest design that obeys the existing `ExtensionContext` boundary and avoids inventing a Node host. It is security-sensitive because approved frontend code still runs in the cockpit origin. |
| Q2 | Which directories and deployment modes are eligible? | **Only direct child package directories under `cezarHomeDir()/extensions`, and only local-handoff mode.** No project trees, task worktrees, remote URLs, archives, or hosted-mode loading. | Reuses `cezarHomeDir()` and `capabilities().localHandoff`; avoids a new setting or environment variable and prevents a remote cockpit from executing the server operator's local extensions in every browser. |
| Q3 | How are requested permissions granted and persisted? | **Empty requests activate with an empty grant. Non-empty requests require an explicit Settings approval.** Persist `{id, version, manifestHash, grantedPermissions}` in a separate Cezar-owned `extension-grants.json`; approval is invalidated when id, version, manifest bytes, or requested permissions change. | A package cannot grant itself permissions. A separate file survives package replacement without making package contents authoritative, and stale approval fails closed. |
| Q4 | Should `entrypoints.backend` execute? | **No. A declared backend entrypoint makes the package incompatible in this phase.** It remains format-valid but fails the existing frontend-only `checkPackageCompatibility` host gate. | The repository has no backend extension host, route ownership model, or sandbox. Refusal is safer and already represented by `unsupported-entrypoint`. |
| Q5 | Should package changes hot-reload? | **No live replacement.** Inventory refresh rescans; approval changes take effect after an explicit cockpit reload. The initial implementation loads packages once per page boot and never replaces an active id. | The current registry has lifecycle operations but no unregister/update transaction. Reload gives every extension a fresh context and makes rollback a directory removal plus reload. |
| Q6 | What diagnostics and HTTP surface are required? | **Workspace-level versioned inventory and approval routes, an existing-settings Extensions section, and logs/toasts for runtime failures.** No SSE/WebSocket topic or persisted diagnostic history in v1. | A query plus explicit refresh is enough for filesystem changes. Existing route chaining, contract schemas, Settings cards, toaster, and `[cezar:extensions]` logging avoid parallel infrastructure. |
...
- `packages/cezar/src/extensions/grants.ts` owns the optional Cezar-managed grant file, atomic read/modify/write, `0600` writes, stale-grant invalidation, and read-only degradation. A corrupt grant file is ignored with one warning and never blocks boot.
...
| `grantedPermissions` | Current grant passed to the runtime; never inferred from a malformed package. |
...
Each entry is keyed by extension id and contains:
...
The hash is an approval-staleness key for the manifest bytes, not an artifact-integrity claim. The loader never treats a package-provided hash as trusted. The store is read-modify-written atomically and with mode `0600`; unknown entries are preserved where the store schema permits. A missing, malformed, or read-only store degrades to no grants, so zero-permission packages can still load and permissioned packages remain pending.
...
| `permission-required` | approve or package change | Approval writes the exact current requested set; any manifest/version/request change returns here. |
| `ready` | page boot | Browser imports, verifies module identity, registers, and activates through the existing host. |
...
| Grant file is missing, corrupt, or stale | Treat all grants as empty; preserve no partially parsed approval. | Permissioned packages return to approval; one warning/log line. |
...
| Package changes after approval | Hash/version/request mismatch invalidates the old grant. | Package returns to permission-required and requires fresh approval. |
...
- There is no migration for a missing grant file. Old Cezar versions ignore the new optional file and do not load the new routes/assets.
- Rollback is deleting the loader wiring/routes and the optional grant file. No project state, run state, workflow format, or extension API type is changed by the loader itself.
- The new HTTP routes, asset route, grant-file format, and local package discovery behavior become compatibility surfaces and must be added to the relevant `BACKWARD_COMPATIBILITY.md` sections in the implementation PR.

### Review limits

This is a design-only spec. No runtime code, route table, tarball, real extension, browser sandbox, or marketplace trust mechanism was changed or exercised here. The high-risk Q1 boundary is explicitly marked for human confirmation before implementation merges.

## 📋 Phasing

1. **Phase 1 - Pure local discovery and grants.** Add path helpers, the grant store, and a Node-side scanner that returns immutable diagnostics using the existing package-manifest validators. It is independently testable and still inert until routes are wired.
2. **Phase 2 - Server inventory and asset boundary.** Add contract schemas, workspace-level GET/PUT routes, safe JS asset serving, local/hosted gating, logs, and route/backward-compatibility inventory. With no packages the application remains unchanged.
...
2. Add the grant store with bounded schema validation, atomic `0600` writes, read-modify-write behavior, stale approval invalidation, corrupt-file recovery, and read-only degradation. Test that a package cannot store a permission it did not request.
...
5. Add `packages/contract/src/extensions.ts`, export its schemas/types, and add api-client helpers/query keys. Add contract parity and typed-route tests before wiring handlers.
6. Add the chained workspace routes for `GET /api/v1/extensions` and `PUT /api/v1/extensions/:id/approval`, using route middleware validators, the existing local-handoff/origin guards, exact response shapes, and 404/409 behavior. Test local, hosted, malformed, unknown-id, and failed-write cases.
...
12. Add the global Settings registry entry and Extensions section with inventory query, status cards, approval action, Check again, Reload cockpit, hosted-mode state, empty state, keyboard/accessibility behavior, and durable `role="alert"` diagnostics. Test each state with the existing toaster and Settings test harness.
