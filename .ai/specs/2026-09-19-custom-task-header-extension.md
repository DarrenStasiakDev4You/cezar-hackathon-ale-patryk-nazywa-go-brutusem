...
and those rows record the decisions. The rest are autonomous defaults, each reversible before
...
- **The first hook in an extension component.** This makes "one React for the cockpit and its
  ...
  author learns it.
- **"Jira" in an example that never talks to Jira (Q6).** The button's label and the panel's
