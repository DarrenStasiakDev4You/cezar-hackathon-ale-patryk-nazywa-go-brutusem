...
| Q2 | Where is the central registry hosted and how is it selected? | **One canonical HTTPS URL, built into Cezar; the document is maintained outside the Cezar binary, preferably as a versioned JSON file in a public Open Mercato GitHub repository.** No custom registry configuration in v1. | A single source keeps discovery zero-config and avoids SSRF/configuration surfaces. The exact production owner and URL need confirmation before implementation. **⚠ NEEDS HUMAN CONFIRMATION** |
...
| Q4 | Is the registry document itself trusted, signed, or merely validated? | **Validated and integrity-checked in transit, but not cryptographically signed in v1.** HTTPS, strict schema validation, bounded payloads and fail-closed entries are required; publisher signatures and registry signing are follow-ups before installation. | The DoD asks for metadata discovery, not execution. A checksum authenticates an artifact's bytes only when obtained from a trusted registry; it does not authenticate the registry or publisher. **⚠ NEEDS HUMAN CONFIRMATION** |
...
| Q6 | Should the fetched list include only versions compatible with this Cezar, or all catalog entries? | **Fetch and validate the complete catalog; return all valid versions with their declared compatibility.** A compatibility verdict is deferred to the installer/loader, which can reuse `checkPackageCompatibility` against the downloaded package manifest. | Discovery and selection are different concerns. Keeping the service as a catalog reader avoids a second semver implementation or a runtime dependency from the published service on the private extension-api package. |
...
The existing `packages/extension-api` remains the source of extension identity, permission vocabulary and compatibility semantics for the future installer/loader. The new marketplace schema lives in `packages/contract` because it crosses the HTTP boundary; the service validates the external document with that schema and the api-client re-exports the inferred types. The service does not reimplement or evaluate semver compatibility in v1, and it must not execute or inspect the artifact to produce the discovery response.
...
- **Canonical source ownership, high.** The repository and exact production URL are not present in the brief or current code. The spec deliberately selects one fixed HTTPS source but marks its ownership and URL for human confirmation before implementation. Do not substitute the current hackathon fork as the production authority.
...
3. **Validate compatibility metadata without duplicating the host decision.** Validate the catalog's `apiVersion` and bounded `cezar` range fields, preserve them in the response, and add fixtures for compatible-looking, out-of-range and pre-release ranges. Do not add a second semver evaluator; the future installer/loader reuses `checkPackageCompatibility` after reading the package manifest.
...
## Security/design blockers

The feature can be specified and implemented as read-only discovery, but the following must be resolved before any installer, automatic update or artifact execution ships:

1. The owner and exact HTTPS URL of the canonical registry must be approved.
2. The project must choose a catalog/publisher authenticity mechanism beyond SHA-256, plus revocation and moderation semantics.
3. The future installer must enforce artifact URL/network safety and cross-check the downloaded package manifest against the catalog before granting permissions or loading code.
