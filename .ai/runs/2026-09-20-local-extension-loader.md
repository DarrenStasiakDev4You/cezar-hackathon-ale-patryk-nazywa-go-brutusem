...
- [x] 4.3 Update AGENTS/reference and BACKWARD_COMPATIBILITY.md as required, then run the full validation and browser smoke gates. — current worktree

Validation note: typecheck, build, unit tests, feature-focused Vitest tests, package installation, and the package gate pass. Manual review found and fixed malformed UTF-8 manifest acceptance in `77892385`. The aggregate `npm test` run remains red on unrelated query-repeat, page-layout, system-prompt, agent-profile, open-in-app, and route-parity timeout failures (8414 passed, 9 failed). Browser smoke was attempted through `npm run test:e2e`; the shared environment was concurrently bootstrapped/reused by another process and the existing cockpit suite failed outside the extension Settings surface. No extension-specific browser scenario exists yet.
