...
Validation note: typecheck, build, unit tests, feature-focused Vitest tests, and package installation pass. Manual review found and fixed malformed UTF-8 manifest acceptance in `77892385`. The aggregate `npm test` run remains red on unrelated automation, system-prompt, agent-profile, open-in-app, and automation-route tests. Browser smoke also remains red/incomplete: `npm run test:e2e` timed out at 180 seconds after three `quick-list.e2e.ts` failures (`groups the runs under Needs you / Recent`, `expands the variant group`, and `is the home`); the test environment was stopped afterward.
